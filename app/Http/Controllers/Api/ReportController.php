        ...
        $data = ReportCacheService::remember(
            ...
            function () use ($params) {
                ...
                $customers = $customersQuery->get()->map(function ($customer) use ($startDate, $endDate, $branchId) {
                    ...
                    if ($branchId) {
                        $ordersQuery->where('projects_id', $branchId);
                    }
    ...
    public function performance(Request $request): JsonResponse
    {
        ...
        $data = ReportCacheService::remember(
            ...
            function () use ($params) {
                ...
                if ($branchId) {
                    $usersQuery->where('projects_id', $branchId);
                }
                if ($userId) {
                ...
                $performanceData = $users->map(function ($user) use ($startDate, $endDate) {
                    ...
                    $attendanceQuery = \App\Models\Attendance::where('users_id', $user->id)
                        ->where('is_deleted', 0);
                    if ($startDate) {
                        $attendanceQuery->where('clock', '>=', $startDate);
                    }
                    if ($endDate) {
                        $attendanceQuery->where('clock', '<=', $endDate);
                    }
                    ...
                    if ($startDate) {
                        $treatmentsQuery->where('date_start', '>=', $startDate);
                    }
                    if ($endDate) {
                        $treatmentsQuery->where('date_start', '<=', $endDate);
                    }
                    ...
                    $notesQuery = \App\Models\DailyNote::where('users_id', $user->id)
                        ->where('is_deleted', 0);
                    if ($startDate) {
                        $notesQuery->where('date', '>=', $startDate);
                    }
                    if ($endDate) {
                        $notesQuery->where('date', '<=', $endDate);
                    }
